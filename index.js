...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const { text, extendedText, contact, location, liveLocation, image, video, , document, audio, product } = MessageType
			...
			switch(command) {
				...
				case '':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Kirim gambar dengan caption ${prefix} atau tag gambar yang sudah dikirim`)
					}
					break
				case 'gtts':
				...
				case 'ademirs':
					...
					break
				case 'toimg':
					if (!isQuoted) return reply('❌ adesivo de resposta um ❌')
					reply(mess.wait)
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ativou Um')
						welkom.push(from)
